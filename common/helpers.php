...
function table_crud_actions($data)
{
  try {
    ...
    if (!isset($data['action'])) {
      $arr_return = ["code" => 500, "success" => false, "messsage" => E_FUNC_ERR, "error" => "No Action"];
      return $arr_return;
      exit();
    }
    if (!in_array($data['action'], $action_allowed)) {
      $arr_return = ["code" => 500, "success" => false, "messsage" => E_FUNC_ERR, "error" => "Invalid Action"];
      return $arr_return;
      exit();
    }
    ...
    if (!isset($data['table_name'])) {
      $arr_return = ["code" => 500, "success" => false, "messsage" => E_FUNC_ERR, "error" => "No Table Name"];
      return $arr_return;
      exit();
    }
    ...
    switch ($table_action) {
      case 'read':
        ...
        $column = $data['columns'];
        if (count($column) == 0) {
          $arr_return = ["code" => 500, "success" => false, "messsage" => E_FUNC_ERR, "error" => "No Columns"];
          return $arr_return;
          exit();
        }
        ...
        if (isset($data['isCondition'])) {
          if ($data['isCondition']) {
            ...
            if (!isset($data['condition_columns']) || !isset($data['columns_data'])) {
              $arr_return = ["code" => 500, "success" => false, "messsage" => E_FUNC_ERR, "error" => "No Conditions"];
              return $arr_return;
              exit();
            }
            ...
            if (count($conditon_column) != count($conditon_column_data)) {
              $arr_return = ["code" => 500, "success" => false, "messsage" => E_FUNC_ERR, "error" => "Conditions column & data don't match"];
              return $arr_return;
              exit();
            }
      ...
      case 'insert':
        ...
        if (count($column) == 0 || count($conditon_column_data) == 0) {
          $arr_return = ["code" => 500, "success" => false, "messsage" => E_FUNC_ERR, "error" => "No Column"];
          return $arr_return;
          exit();
        }
        if (count($column) != count($conditon_column_data)) {
          $arr_return = ["code" => 500, "success" => false, "messsage" => E_FUNC_ERR, "error" => "Column & data don't match"];
          return $arr_return;
          exit();
        }
      ...
      case 'update':
        ...
        $conditon_column_data = $data['columns_data'];
        if (count($column) == 1) {
          $arr_return = ["code" => 500, "success" => false, "messsage" => E_FUNC_ERR, "error" => "Should be more than 1 column."];
          return $arr_return;
          exit();
        }
        if (count($column) == 0 || count($conditon_column_data) == 0) {
      ...
      case 'delete':
        ...
        $conditon_column_data = $data['columns_data'];
        if (count($column) > 1) {
          $arr_return = ["code" => 500, "success" => false, "messsage" => E_FUNC_ERR, "error" => "Should not be more than 1 column."];
          return $arr_return;
          exit();
        }
        if (count($column) == 0 || count($conditon_column_data) == 0) {
...
function get_page_access_by_pageid($companycode, $email, $role, $type, $pageid)
{
  ...
  try {
    ...
    $result = $session->execute($session->prepare("SELECT createaccess,modifyaccess,viewaccess FROM roletocustomer WHERE rtccustemail=? AND rtcrole=? AND rolestatus=? AND companycode=? ALLOW FILTERING"), array(
      'arguments' => array(
        $email,
        $role,
        "1",
    ...
    foreach ($result as $row) {
      switch ($type) {
        case 'create':
          $pageAccess = explode("|", $row['createaccess']);
          array_shift($pageAccess);
          break;
        case 'modify':
          $pageAccess = explode("|", $row['modifyaccess']);
          array_shift($pageAccess);
          break;
        case 'view':
          $pageAccess = explode("|", $row['viewaccess']);
          array_shift($pageAccess);
          break;
        case 'all':
          $createaccess = explode("|", $row['createaccess']);
          array_shift($createaccess);
          $modifyaccess = explode("|", $row['modifyaccess']);
          array_shift($modifyaccess);
          $viewaccess = explode("|", $row['viewaccess']);
          array_shift($viewaccess);
          $pageAccess = array_merge($createaccess, $modifyaccess, $viewaccess);
          break;
      }
    }

    if (in_array($pageid, $pageAccess)) {
      $access = true;
    }
...
function get_name_from_email($email)
{
  try {
    ...
    $result = $session->execute($session->prepare("SELECT custfname,custlname FROM customer WHERE custemailaddress=?"), array(
      'arguments' => array(
        $email
      )
    )
...
function get_name_and_email_from_custcode($custcode)
{
  try {
    ...
    $result = $session->execute($session->prepare("SELECT custfname,custlname,custemailaddress FROM customer WHERE custcode=? ALLOW FILTERING"), array(
      'arguments' => array(
        $custcode
      )
    )
...
function get_custcode_from_email($email)
{
  try {
    ...
    $result = $session->execute($session->prepare("SELECT custcode FROM customer WHERE custemailaddress=?"), array(
      'arguments' => array(
        $email
      )
    )
...
function notice_write($notice_module_id, $companycode, $notice_from, $notice_from_role, $notice_link, $notice_to, $notice_to_role, $txn_name, $transactionid, $txn_name2 = "")
{
  ...
  try {
    ...
    $query_insert_in_company = $session->prepare('INSERT INTO notice(
      ...
      VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)');
    $session->execute($query_insert_in_company, array(
      'arguments' => array(
        new \Cassandra\Uuid(),
        $companycode,
...
function notice_update($transactionid, $companycode, $notice_to, $notice_to_role, $notice_module_id)
{
  try {
    global $session;
    $result_notice = $session->execute($session->prepare('SELECT notice_no,notice_to,notice_to_role FROM notice WHERE companycode=? AND transactionid=? AND notice_to=? AND notice_to_role=? AND notice_module_id=? ALLOW FILTERING'), array(
      'arguments' => array(
        $companycode,
        $transactionid,
        $notice_to,
    ...
    if ($result_notice->count() == 0) {
      $result_notice = $session->execute($session->prepare('SELECT notice_no,notice_to,notice_to_role FROM notice WHERE companycode=? AND transactionid=? AND notice_to=? AND notice_to_role=? AND notice_module_alt=? ALLOW FILTERING'), array(
        'arguments' => array(
          $companycode,
          $transactionid,
          $notice_to,
    ...
    foreach ($result_notice as $row_notice) {
      ...
      if ($row_notice['notice_to'] == "" || $row_notice['notice_to_role'] == "") {
      } else {
...
function module_assign_email_role_list($page_module, $module_type, $companycode)
{
  try {
    global $session;
    $arr = array();
    if ($page_module == '' || $module_type == '' || $companycode == '') {
      $arr[0] = array("email" => "", "role" => "");
      return $arr;
      exit();
    }
...
function insert_data_into_client_product_subscription($product, $companycode, $expirydate, $unit, $balance, $currency, $email, $role, $custcode)
{
  try {
    ...
    if ($result_pr->count() == 0) {
      return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "Invalid Product for Subscription"];
      exit();
    }

    $get_company_tier_details = get_company_tier_details($companycode);
    if (!$get_company_tier_details['success']) {
      return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "Invalid Company Tier"];
      exit();
    }
    ...
    foreach ($result_pr as $row_pr) {
      ...
      $columns_data = [
        ...
        $row_pr['moduleid'],
        $row_pr['version'],
        $row_pr['packageversion'],
        $row_pr['moduleversion'],
        $row_pr['product_key'],
        $row_pr['product'],
        $companycode,
        $email,
...
function check_if_product_is_valid($product)
{
  try {
    global $session;
    if ($product == "") {
      $arr_return = ["success" => false, "msg" => "Invalid Product"];
      return $arr_return;
      exit();
    }
    ...
    $arr_return = ["success" => false, "msg" => "Error Occured"];
    return $arr_return;
  }
}
...
function update_company_expiry_by_laws_expiry_date($companycode)
{
  try {
    global $session;
    if ($companycode == "") {
      $arr_return = ["success" => false, "msg" => "Invalid Company"];
      return $arr_return;
      exit();
    }
    ...
    if ($result_pr->count() == 0) {
      $arr_return = ["success" => false, "msg" => "No Law Found"];
      return $arr_return;
      exit();
    }
    $arr_dates = [];
    ...
    $arr_return = ["success" => true, "msg" => "Updated"];
    return $arr_return;
  } catch (\Exception $e) {
    $arr_return = ["success" => false, "msg" => "Error Occured"];
    return $arr_return;
...
function get_company_tier_details($companycode)
{
  try {
    ...
    $arr_return = ["success" => true, "msg" => "success", "data" => $data];
    return $arr_return;
...
function isDateInRange($dateString, $minDate = null, $maxDate = null)
{
  ...
  if (
    ($minDate === null || $inputDate >= new DateTime($minDate)) &&
  ...
  try {
    switch ($type) {
      ...
      case 'vendor':
        ...
        break;
      case 'technical risk':
        $result = $session->execute($session->prepare("SELECT rolename FROM rolematrix WHERE techriskstatus=? ALLOW FILTERING"), array('arguments' => array("1")));
        foreach ($result as $row) {
...
function get_vendor_name_by_vendor_id($vendorid)
{
  try {
    ...
    if ($result_vendor->count() > 0) {
      $vendor = $result_vendor[0]['vccustname'];
    }
    return $vendor;
  } catch (\Exception $e) {
    return "";
