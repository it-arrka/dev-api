    ...
    switch ($funcCallType) {
      ...
      case "initiateAssessment":
        ...
        if (isset($GLOBALS['companycode']) && isset($json['email']) && isset($json['role']) && isset($json['transactionid']) && isset($GLOBALS['email']) && isset($GLOBALS['role'])) {
          ...
          if ($json['email'] == 'default') {
            $json['email'] = $GLOBALS['email'];
          }
          if ($json['role'] == 'default') {
            $json['role'] = $GLOBALS['role'];
          }
...
function get_maturity_report_call($companycode, $txn_id, $limit, $page)
{
  try {
    global $session;
    if ($txn_id == "") {
      return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "Invalid company"];
      exit();
    }
    ...
    if ($result_txn->count() == 0) {
      return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "Invalid transaction"];
      exit();
    }
    ...
    if ($acf_flag == "1") {
      ...
      return $get_maturity_report_acf;
      exit();
    } else {
      ...
      return $get_maturity_report_old;
      exit();
    }
...
function get_maturity_report_acf($companycode, $txn_id, $version, $limit, $page)
{
  try {
    ...
    foreach ($result_txn as $row_txn) {
      ...
      if ($gquestionno_act == '') {
        $gquestionno_act = $row_txn['gapanalysisid'];
      }
    ...
    $arr_txn_sec = [];
    foreach ($arr_txn_init as $element) {
      $arr_txn_sec[$element['gdomain']][] = $element;
    }
    ...
    foreach ($arr_final_txn as $value_txn) {
      ...
      foreach ($result as $row) {
        ...
        $doc_id = explode("|", $row['gdocid']);
        $doc_name = explode("|", $row['gdocname']);
        array_shift($doc_id);
        array_shift($doc_name);
        if ($row['remark'] == "") {
          $row['remark'] = "";
        }
...
function get_maturity_report_old($companycode, $txn_id, $type, $lawInput, $limit, $page)
{
  try {
      ...
      switch ($row['gscore']) {
        case '0':
          ...
          foreach ($result_actions as $row_actions) {
            if ($row_actions['validation_status'] == 'Risk Accepted') {
              $row_actions['action_status'] = "Risk Accepted";
            } else {
              array_push($action_status, $row_actions['action_status']);
            }
          }
          if (count($action_status) > 0) {
            if (in_array("close", $action_status)) {
              if (count(array_unique($action_status)) == 1) {
                $updated_status = "Compliant";
                $updated_score = '1';
              } else {
                $updated_status = "Non Compliant";
              }
            } else {
              $updated_status = "Non Compliant";
            }
    ...
    if ($lawInput == '') {
      ...
      foreach ($result_lcontrol as $row_lcontrol) {
        array_push($lcontrol, $row_lcontrol['lcontrolno']);
      }
    }
    ...
    //Sort first array
    foreach ($gapanalysis_data_array as $temp_key_for_ques => $row_for_required_ques) {
      $temp_price_for_ques[$temp_key_for_ques] = $row_for_required_ques["sequence"];
    }
  ...
  try {
    ...
    foreach ($result_txn as $row_txn) {
      if ($row_txn['status'] != 'disable') {
        if ($row_txn['transactiontype'] == "vendor" || $row_txn['transactiontype'] == "technical risk" || $row_txn['transactiontype'] == "privacy_notice") {
        } else {
    ...
    foreach ($arr_final_txn as $tid => $value) {
      ...
      foreach ($result as $row) {
        ...
        if ($result_keys->count() > 0) {
          $txn_type_show = $result_keys[0]['dispname'];
        }
        ...
        if ($result_action_status->count() > 0) {
        ...
        if ($createdate == "") {
          ...
          $create_date = date("d-m-Y", (int) $createdate / 1000);
        }
        ...
        if ($result_law->count() > 0) {
          $active_law_for_tid = $result_law[0]['dispname'];
        }
...
function get_user_role_list_by_law($companycode, $law, $type)
{
  try {
    ...
    if ($get_roles_for_laws_by_acf['success']) {
      $role_arr = $get_roles_for_laws_by_acf['data'];
    }
    $arr = [];
...
function maturity_transaction_write($companycode, $email, $role, $activeLaw, $transaction_array)
{
  ...
  try {
    ...
    if (!is_array($transaction_locationinscope)) {
      return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "invalid location in scope"];
      exit();
    }
    ...
    if (!is_array($role_email_arr)) {
      return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "invalid role_email_map"];
      exit();
    }
    ...
    if (!is_array($transaction_department)) {
      return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "invalid department"];
      exit();
    }
    if (count($transaction_department) == 0) {
      return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "Select Department"];
      exit();
    }
    ...
    foreach ($result_version as $row_version) {
      array_push($version_array, (int) $row_version['version']);
    }
    ...
    foreach ($role_email_arr as $keyRole => $check_arr) {
      ...
      if (sizeof($check_arr) == 0) {
        return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "Atleast one email must be checked under each role!"];
        exit();
      }
    ...
    if (!checkKeysExist($allRolesReq, $roleIncoming)) {
      return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "send all required role and email. All roles " . implode(', ', array_keys($allRolesReq))];
      exit();
    }

    if ($transaction_name == '') {
      return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "Fill Assesment Name"];
      exit();
    }

    if ($transaction_law == '') {
      return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "Select Assessment Law"];
      exit();
    }

    if (count($transaction_locationinscope) == 0) {
      return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "Select Location in Scope"];
      exit();
    }
    ...
    foreach ($role_email_arr as $key_a => $val_a) {
      foreach ($val_a as $val_f) {
        ...
        if ($result_temp_notice->count() > 0) {
          ...
          $session->execute(
            $session->prepare('INSERT INTO temp_notice (id,email,role,law,assessment_notice,companycode,createdate,effectivedate) VALUES(?,?,?,?,?,?,?,?)'),
            array(
              'arguments' => array(
                new \Cassandra\Uuid(),
                $val_f,
        ...
        $result_submit = $session->execute(
          "SELECT modifyaccess,rtcuuid FROM roletocustomer WHERE rtcrole=? AND rtccustemail=? AND rolestatus=? AND companycode=? ALLOW FILTERING",
          array(
            'arguments' => array(
              $key_a,
              $val_f,
        ...
        foreach ($result_submit as $row_submit) {
          $modifyaccess = explode("|", $row_submit['modifyaccess']);
          if ($row_submit['modifyaccess'] == "") {
            ...
            if (strpos($row_submit['modifyaccess'], 'PG008') !== false) {
            } else {
...
function assessmentstatus_write($companycode, $email, $role, $transactionid, $reload = true)
{
  try {
    ...
    if (!check_if_email_role_exist_in_company($companycode, $email, $role)) {
      return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "invalid role & email"];
      exit();
    }

    $acf_flag = "";
    $type_of_txn_table = "transactions";
    $lawSel = "";
    $email_role_status = ' ';
    $version_to_txn = ' ';
    $assessment_type_alt = '';
    ...
    if ($result_txn_chk->count() == 0) {
      ...
      if ($result_txn_chk->count() == 0) {
        return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "invalid transactionid"];
        exit();
      }
    ...
    if ($data['type'] == "privacy_notice") {
      ...
      foreach ($result as $a_v) {
        $assessmentversion = $a_v['assessmentversion'];
        $testid = $a_v['testid'];
      }
      if ($testid == "") {
      ...
      if ($email_role_status == '1') {
        ...
        if ($result_er->count() == 0) {
          return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "invalid role & email for this transaction"];
          exit();
        }
      ...
      if ($testid == "") {
        $testid = $transactionid . "-" . $role . "-" . $data['type'] . "-" . $custcode;
      }
      $count = $result->count();
    }
    if ($count == 0) {
      //check for type_of_txn_table
      if ($type_of_txn_table == "transactions" || $type_of_txn_table == "internal") {
      } else {
        return ["code" => 400, "success" => false, "message" => E_INV_REQ, "error" => ""];
        exit();
      }
...
function load_question_to_response_acf($companycode, $custcode, $email, $transactionid, $type_of_txn_table, $type, $role, $testid, $version_to_txn, $testname, $createdate, $vendor)
{
  ...
  if ($type_of_txn_table == 'transactions') {
    ...
    if (!$load_assessment_question_by_acf['success']) {
      return $load_assessment_question_by_acf;
      exit();
    }
...
function check_for_question_in_assessment($companycode, $email, $role, $transactionid, $assessmentstatus_write_arr, $assessmentversion, $testid_t_ff, $acf_flag)
{
  try {
    ...
    switch ($assessmentstatus_write_arr['type']) {
      case 'vendor':
        if ($assessmentversion == 'new') {
          $result = $session->execute($session->prepare('SELECT testid FROM assessmentstatus WHERE transactionid=? AND role=? AND custemail=? ALLOW FILTERING'), array('arguments' => array(new \Cassandra\Uuid($transactionid), $role, $email)));
        } else {
...
function get_maturity_question($assessmentid, $limit, $page)
{
  ...
  try {
    if (!isValidBase64($assessmentid)) {
      return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "Invalid testid"];
      exit();
    }
    ...
    if ((string) $transactionid == "") {
      return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "invalid maturity transaction"];
      exit();
    }
    ...
    foreach ($arr_final_txn as $questionno) {
      ...
      foreach ($res as $tmp) {
        ...
        foreach ($result as $row) {
          ...
          $docid_arr = explode("|", $tmp['docid']);
          array_shift($docid_arr);
          $docname_arr = explode("|", $tmp['docname']);
          array_shift($docname_arr);
...
function save_assessment($companycode, $email, $role, $assessmentid)
{
  ...
  try {
    ...
    if (!isValidBase64($assessmentid)) {
      return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "Invalid assessmentid"];
      exit();
    }
    $assessmentid = base64_decode($assessmentid);
    $assessmentid = escape_input($assessmentid);
    ...
    if ($assessment_email != $email || $assessment_role != $role) {
      return ["code" => 403, "success" => false, "message" => E_NO_PAGE_ACCESS, "error" => "You do not have access to this assessment"];
      exit();
    }
    ...
    if ($acf_flag == "1") {
      ...
      return $assessment_commit_by_acf;
      exit();
    }
    ...
    foreach ($result_chk as $chk) {
      if ($chk['status'] == "1" || $chk['status'] == '2') {
        return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "Already submitted by another user"];
        exit();
      }
    }
    ...
    foreach ($result_from_temp_gap_for_commit as $row) {
      $uuid_for_commit = new \Cassandra\Uuid();
      $controlno = "controlno";
      $arrkarefno = "arrkarefno";
      $domainno = "domainno";
      ...
      if ($row['version'] == '4' || $row['version'] == '5') {
        ...
        if ($type_test == 'security' || $type_test == 'privacy') {
          ...
          foreach ($result_from_ref_for_commit as $row_ref) {
            $controlno .= "," . $row_ref['armcontrolno'];
            $arrkarefno .= "," . $row_ref['armarrkarefno'];
          ...
          foreach ($result_from_ref_for_commit as $row_ref) {
            ...
            $controlno .= "," . $result_from_ref_for_commit[0]['lawcontrolno'];
            $domainno .= "|" . $result_from_ref_for_commit[0]['lawdomain'];
          }
        }
      }
      ...
      foreach ($result_from_question_list_for_commit as $row_ques) {
        $question_to_update = $row_ques['question'];
      }
      //handle mcq
      $mcq = '';
      if ($row['mcq'] == '') {
      } else {
        $mcq_arr = explode("|", $row['mcq']);
        for ($i = 1; $i < sizeof($mcq_arr); $i++) {
          $mcq .= " " . substr($mcq_arr[$i], 5);
        }
      }
      ...
      foreach ($result_from_testtype as $row_testtype) {
        $gtesttype = $row_testtype['transactiontype'];
      }
    ...
    if ($result_txn[0]['email_role_status'] == '1') {
      ...
      if ($result_em_role->count() == $result_s->count()) {
        $email_role_status = "1";
      }
    } else {
      if (count(rolematrix_for_assessment($row['transactiontype'])) == $result_s->count()) {
        $email_role_status = "1";
      }
    }
...
function temp_save_response($companycode, $email)
{
  try {
    ...
    if (isset($_POST['testid']) && isset($_POST['type']) && isset($_POST['questionno'])) {
      ...
      return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "invalid testid, questionno or type"];
      exit();
    }
    ...
    switch ($type) {
      case "radio_response":
        if (!isset($_POST['response']) || !isset($_POST['score']) || !empty($_POST['response']) || !empty($_POST['score'])) {
          return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "invalid request; response or score"];
          exit();
        }
        ...
        if (!in_array($score, ['NA', '0', '1', '2', '3'])) {
          return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "invalid score"];
          exit();
        }
      ...
      case "text_response":
        if (!isset($_POST['response']) || !isset($_POST['score']) || !isset($_POST['type_for_text']) || !empty($_POST['response']) || !empty($_POST['score']) || !empty($_POST['type_for_text'])) {
          return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "invalid request; response, score or type_for_text"];
          exit();
        }
        ...
        if (!in_array($type_for_text, ['response', 'remark'])) {
          return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "invalid type_for_text"];
          exit();
        }
      ...
      case "upload_response":
        if (!isset($_FILES["files"])) {
          return ["code" => 400, "success" => false, "message" => E_PAYLOAD_INV, "error" => "invalid files request"];
          exit();
        }
        ...
        $session->execute(
          $session->prepare("UPDATE " . $query_table . " SET docid=? ,docname=?,modifydate=? WHERE assessmentid=? AND questionno=?"),
          array(
            'arguments' => array(
              $docid . $docid_n,
              $docname . $docname_n,
