...
function get_user_activity_info($email, $companycode, $role, $limit, $day)
{
  try {
    ...
    $result = $session->execute($session->prepare("SELECT createdate,modifydate,icon_status,notice_to_role FROM notice WHERE notice_to = ? AND companycode=? AND status=? AND notice_status=? ALLOW FILTERING"), array(
      'arguments' => array(
        $email,
        $companycode,
        "1",
    ...
    foreach ($result as $row) {
      ...
      if ($modifydate_str == "") {
        $modifydate_str = (string) $row['createdate'];
      }
      ...
      if ($modifydate_int >= $timestamp) {
        ...
        if ($row['notice_to_role'] == $role) {
          $role_open++;
        }
      }
    }

    $result_closed = $session->execute($session->prepare("SELECT createdate,modifydate FROM notice WHERE notice_to = ? AND companycode=? AND status=? AND notice_status=? ALLOW FILTERING"), array(
      'arguments' => array(
        $email,
        $companycode,
        "1",
    ...
    foreach ($result_closed as $row_closed) {
      ...
      if ($modifydate_str == "") {
        $modifydate_str = (string) $row_closed['createdate'];
      }
      $modifydate_int = (int) $modifydate_str / 1000;
      if ($modifydate_int >= $timestamp) {
        $total_activity++;
...
function get_user_activity($email, $companycode, $role, $limit, $page, $day)
{
  try {
    ...
    $result = $session->execute($session->prepare("SELECT notice_no,createdate,modifydate,icon_status FROM notice WHERE notice_to = ? AND notice_to_role=? AND companycode=? AND status=? AND notice_status=? ALLOW FILTERING"), array(
      'arguments' => array(
        $email,
        $role,
        $companycode,
    ...
    foreach ($arr_final_txn as $notice_no => $createdate) {
      $result_assign = $session->execute($session->prepare("SELECT * FROM notice WHERE notice_no = ?"), array(
        'arguments' => array(
...
function get_user_activity_all($email, $companycode, $limit, $page, $day)
{
  try {
    ...
    $result = $session->execute($session->prepare("SELECT notice_no,createdate,modifydate,icon_status FROM notice WHERE notice_to = ? AND companycode=? AND status=? AND notice_status=? ALLOW FILTERING"), array(
      'arguments' => array(
        $email,
        $companycode,
        "1",
