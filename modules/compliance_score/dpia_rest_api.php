...
function read_from_compliance_score_dpia($companycode,$email,$role,$custcode){
    try {
      ...
      errorLog($_SERVER['REMOTE_ADDR'],"ER003","database error:compliance_score_dpia","1",(string)$e,"",$_SERVER['PHP_SELF'],$_SERVER['HTTP_REFERER'],session_id(),http_response_code(),$role,$_SERVER['HTTP_USER_AGENT'],$email,$custcode,$companycode);
...
function update_compliance_score_for_dpia($companycode,$email,$role,$custcode){
    try {
     ...
     errorLog($_SERVER['REMOTE_ADDR'],"ER003","database error:update_compliance_score_for_dpia","1",(string)$e,"",$_SERVER['PHP_SELF'],$_SERVER['HTTP_REFERER'],session_id(),http_response_code(),$role,$_SERVER['HTTP_USER_AGENT'],$email,$custcode,$companycode);
     $arr_return=["success"=>false,"msg"=>"Error Occured $e","data"=>(string)$e];
     return $arr_return;
   }
 }
