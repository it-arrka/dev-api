...
function get_policy_domains_data($law_tid, $domains, $companycode, $email, $role){
  try {
    ...
    $result_version= $session->execute($session->prepare('SELECT createdate,version FROM policyacfclientmapping_version WHERE law_tid=? AND law_version=? AND companycode=? AND status=? AND domains=?'),array('arguments'=>array(
      $law_tid,$law_version,$companycode,"1",$domains
    )));
  ...
  try {
    ...
    foreach ($result_map as $row_map) {
      ...
      if ($row_map['domains_acf']==$domains && $row_map['policy_version']==$policy_edit_version) {
        ...
        $session->execute($session->prepare('DELETE FROM policyacfclientmapping WHERE status=? AND version=? AND law_tid=? AND law_version=? AND companycode=? AND sequence=? AND id=?'),array('arguments'=>array(
          "1",$row_map['version'],$law_tid,$law_version,$companycode,$row_map['sequence'],$row_map['id']
        )));
...
function delete_policy_domains($law_tid, $domains,$domains_text, $companycode, $email, $role){
  try {
    ...
    if($policy_active_version==0){ $policy_active_version=1; }
  ...
  try {
    ...
    if ($domains_acf=='add_new') {
      ...
      if ($result_domain->count()==0) {
        $result_domain= $session->execute($session->prepare('SELECT version FROM policyacfclientmapping_version WHERE law_tid=? AND law_version=? AND companycode=? AND status=? AND domains=?'),array('arguments'=>array(
          $law_tid,$law_version,$companycode,"1",$domains_acf
        )));
        if ($result_domain->count()==0) {
    ...
    //Get domain version
    $result_version= $session->execute($session->prepare('SELECT version FROM policyacfclientmapping_version WHERE law_tid=? AND law_version=? AND companycode=? AND status=? AND domains=?'),array('arguments'=>array(
      $law_tid,$law_version,$companycode,"1",$domains_acf
    )));
    ...
    foreach ($polData as $key_val => $value_val) {
      ...
      foreach ($stmt_arr as $key => $value) {
        ...
        $status="0"; if ($value['selected']) { $status="1"; }
...
function check_if_all_domains_are_saved($law_tid, $companycode, $email, $role){
  try {
    ...
    $domains_not_saved=[]; $saved_domains=[]; $saved=true;
    foreach ($domain_data as $value) {
      if (!$value['saved']) { $saved=false;
